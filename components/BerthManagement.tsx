...
const BerthManagement: React.FC<BerthManagementProps> = ({ onBack }) => {
  ...
  const handleScanTag = async (field: string) => {
    ...
    // Aborta qualquer leitura anterior para garantir que não haja duplicidade de listeners
    if (nfcAbortControllerRef.current) {
      nfcAbortControllerRef.current.abort();
    }
    ...
    reader.onload = async (event) => {
      try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          if (leitoId) {
            detailsMap[leitoId] = {
              crewName: String(row[NOME_COL_IDX] || '').trim().toUpperCase(),
              role: String(row[FUNCAO_COL_IDX] || '').trim().toUpperCase(),
              company: String(row[EMPRESA_COL_IDX] || '').trim().toUpperCase()
            };
            count++;
          }
        }
        ...
        if (count > 0) {
          ...
          await loadBerths();
          alert(`${count} leitos atualizados.`);
        }
      } catch (err) {
    ...
    <div className="flex-1 flex flex-col p-4 md:p-6 max-w-7xl mx-auto w-full pb-40 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-200 text-slate-600 shadow-sm active:scale-90 transition-all">
            <i className="fa-solid fa-chevron-left"></i>
          </button>
          <div>
            <h2 className="text-lg md:text-xl font-black text-slate-900 uppercase leading-none">Gestão de POB</h2>
            <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1">Sincronização Avançada</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-2">
           <button onClick={handleOpenAdd} className="flex-1 md:flex-none bg-emerald-600 text-white px-4 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-lg active:scale-95 flex items-center justify-center gap-2">
           ...
           <button onClick={() => fileInputRef.current?.click()} className="flex-1 md:flex-none bg-blue-600 text-white px-4 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-lg active:scale-95 flex items-center justify-center gap-2">
             <i className="fa-solid fa-file-excel"></i> Importar
           </button>
           ...
           <button onClick={() => setIsConfirmingClear(true)} className="flex-1 md:flex-none bg-rose-50 text-rose-600 px-4 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest border border-rose-100 active:scale-95 flex items-center justify-center gap-2">
             <i className="fa-solid fa-user-minus"></i> Limpar
           </button>
        </div>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 md:p-8">
          <div className="bg-white rounded-[32px] md:rounded-[48px] max-w-2xl w-full p-6 md:p-10 shadow-2xl animate-in zoom-in duration-300 relative overflow-y-auto max-h-[90vh]">
            ...
            {isScanning && (
              <div className="fixed inset-0 bg-blue-600/95 backdrop-blur-md z-[120] flex flex-col items-center justify-center p-10 text-center animate-in fade-in duration-300">
                <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
                  <i className="fa-solid fa-nfc text-4xl text-white"></i>
                </div>
                ...
                <button onClick={stopNfcScan} className="mt-10 px-8 py-3 bg-white text-blue-600 rounded-2xl font-black text-[10px] uppercase active:scale-95">Cancelar</button>
              </div>
            )}

            <div className="flex justify-between items-center mb-6 md:mb-10">
              <h3 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tight">EDITAR LEITO</h3>
              <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:text-slate-600 transition-all">
            ...
            <form onSubmit={handleAddManual} className="space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      <input type="text" value={newBerth.tagId1 || ''} onChange={e => setNewBerth({...newBerth, tagId1: e.target.value.toUpperCase()})} className="w-full pl-5 pr-10 py-3 md:py-4 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-mono font-bold uppercase outline-none" placeholder="Opcional" />
                      {newBerth.tagId1 && (
                    ...
                    <button type="button" onClick={() => handleScanTag('tagId1')} className="w-12 h-12 md:w-14 md:h-14 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg active:scale-90 transition-all">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      <input type="text" value={newBerth.tagId2 || ''} onChange={e => setNewBerth({...newBerth, tagId2: e.target.value.toUpperCase()})} className="w-full pl-5 pr-10 py-3 md:py-4 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-mono font-bold uppercase outline-none" placeholder="Opcional" />
                      {newBerth.tagId2 && (
                    ...
                    <button type="button" onClick={() => handleScanTag('tagId2')} className="w-12 h-12 md:w-14 md:h-14 bg-slate-100 text-slate-400 rounded-2xl flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all active:scale-90">
                      <i className="fa-solid fa-wifi text-lg"></i>
                    </button>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      <input type="text" value={newBerth.tagId3 || ''} onChange={e => setNewBerth({...newBerth, tagId3: e.target.value.toUpperCase()})} className="w-full pl-5 pr-10 py-3 md:py-4 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-mono font-bold uppercase outline-none" placeholder="Opcional" />
                      {newBerth.tagId3 && (
                    ...
                    <button type="button" onClick={() => handleScanTag('tagId3')} className="w-12 h-12 md:w-14 md:h-14 bg-slate-100 text-slate-400 rounded-2xl flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all active:scale-90">
                      <i className="fa-solid fa-wifi text-lg"></i>
                    </button>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="space-y-1">
                  ...
                  <input type="text" value={newBerth.role} onChange={e => setNewBerth({...newBerth, role: e.target.value.toUpperCase()})} className="w-full px-5 py-3 md:py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-black uppercase outline-none" placeholder="RADIO OPERADOR" />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">EMPRESA</label>
                  <input type="text" value={newBerth.company} onChange={e => setNewBerth({...newBerth, company: e.target.value.toUpperCase()})} className="w-full px-5 py-3 md:py-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-black uppercase outline-none" placeholder="SOLSTAT" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 pb-6">
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">BALEIRA PRIMÁRIA</label>
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">BALEIRA SECUNDÁRIA</label>
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-slate-50">
                <button type="submit" disabled={isLoading} className="w-full py-4 md:py-5 bg-blue-600 text-white font-black rounded-[20px] text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all">
                  {isLoading ? 'SALVANDO...' : 'SALVAR ALTERAÇÕES'}
                </button>
                <button type="button" onClick={() => setIsModalOpen(false)} className="w-full py-4 md:py-5 bg-slate-100 text-slate-500 font-black rounded-[20px] text-[10px] uppercase tracking-widest active:scale-95">
                  CANCELAR
                </button>
              </div>
            </form>
      ...
      {isConfirmingClear && (
        <div className="fixed inset-0 z-[200] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-6 text-center">
          <div className="bg-white rounded-[40px] max-w-sm w-full p-10 md:p-12 shadow-2xl animate-in zoom-in duration-300">
            ...
            <div className="grid gap-3">
              <button onClick={async () => { await cloudService.clearBerthNames(); await loadBerths(); setIsConfirmingClear(false); }} className="w-full py-4 bg-rose-600 text-white font-black rounded-3xl text-[10px] uppercase shadow-xl active:scale-95 transition-all">Sim, Limpar</button>
